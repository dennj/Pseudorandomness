...
\paragraph{Lean statement (excerpt).}
In Lean (\texttt{Pseudorandomness/Core/ObserverBarrier.lean}), the main theorem is:
\begin{verbatim}
theorem observer_barrier (O : ObserverClass n)
    (f : BoolFun n) (hPR : IsPseudorandomTo f O) :
    forall T : ProofTechnique n, T.isOBounded O ->
    not (T.certifiesSeparation /\
      exists obs, obs ∈ T.observerClass /\
...
In the Lean development, we do \emph{not} mechanize the cryptographic reduction (OWF $\Rightarrow$ PRF / GGM). Instead, we take the existence of a function pseudorandom to $\mathrm{PolyTimeObservers}(n,k)$ as a hypothesis (``PRF existence'') and derive the barrier theorem as a pure logical implication.
...
\begin{verbatim}
theorem natural_proofs_barrier {n : Nat} (k : Nat)
    (f : BoolFun n) (hPR : IsPseudorandomTo f (PolyTimeObservers n k)) :
    forall T : ProofTechnique n, T.isOBounded (PolyTimeObservers n k) ->
    not (T.certifiesSeparation /\
      exists obs, obs ∈ T.observerClass /\
...
Our Lean formalization captures this connection in \texttt{Pseudorandomness/Applications/MachineLearning/MachineLearningBridge.lean}, proving:
\begin{verbatim}
theorem carmosino_from_observer_barrier (k : Nat)
    (hPRF : exists f : BoolFun n, IsPseudorandomTo f (PolyTimeObservers n k)) :
    exists f : BoolFun n,
      (forall obs, obs ∈ PolyTimeObservers n k -> not (Distinguishes obs f)) := by
\end{verbatim}

\textbf{Note}: The Lean development focuses on the observer-theoretic core (distinguishing vs.\ pseudorandomness) and does not implement the full constructive reductions of Carmosino et al.
...
We prove in \texttt{Pseudorandomness/Applications/ControlTheory/ControlTheoryBridge.lean}:
...
\paragraph{Lean statement (excerpt).}
In Lean, the bridge theorem is:
\begin{verbatim}
theorem observability_bridge :
    Iff sys.IsObservable
      (forall x : Fin n -> 𝕜, sys.IsPseudorandomToOutputs x -> x = 0) := by
  ...
\end{verbatim}
...
The project is managed by Lake; \texttt{lean-toolchain} pins Lean to \texttt{leanprover/lean4:v4.27.0-rc1}, and the library builds with \texttt{lake build}.
This includes:
\begin{itemize}
